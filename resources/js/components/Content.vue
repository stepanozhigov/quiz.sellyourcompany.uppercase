				...
				<div class="questContent__items">
							...
							<li>
								<label for="opt3-2" class="label">
									<input
										...
										id="opt3-2"
										class="label_input"
										value="Производства горных работ"
									/>
									<span class="radio"></span>
						...
						<ul class="questContent_list">
							<li>
								<label for="opt6-1" class="label">
									<input
										type="radio"
										v-model="questions.quest_6"
										id="opt6-1"
										class="label_input"
										value="Общеустановленный"
									/>
									<span class="radio"></span>
					...
					<div class="questContent__items_item" v-if="step == 10">
						<label
							for="opt10_1"
							class="label_text"
							v-html="$ml.get('opt10_1')"
						/>
						<input
							type="text"
							v-model="questions.quest_10"
							id="opt10_1"
							class="input"
					...
					<div class="questContent__items_item" v-if="step == 11">
						...
						<p
							class="label_text label_text_two"
							v-html="$ml.get('opt11_label_2')"
						/>
...
<script>
	...
	const binValidat = helpers.regex(
		"binValidat",
		/^\(?\d{4}\)?[ ]?(\d{4})[ ]?(\d{4})$/
	);
	...
	export default {
		...
		methods: {
			...mapActions(["nextStep", "prevStep", "skip", "callBack"]),
			next(s) {
				if (s === 1) {
			...
			submitQuiz(e) {
				...
				if (this.isValidPhone) {
					let text =
						`Номер БИН компании: ${this.questions.quest_1}\n` +
						`Наличие лицензии: ${this.questions.quest_2}\n` +
						`Лицензия в сфере: ${
							this.questions.quest_3 == "Другое"
								? this.questions.quest_3_text
								: this.questions.quest_3
						}\n` +
						`Наличие подтверждённого опыта работ: ${
							this.questions.quest_4 == "Другое"
								? this.questions.quest_4_text
								: this.questions.quest_4
						}\n` +
						`Наличие оборотов по расчетному счёту: ${
							this.questions.quest_5 == "Другое"
								? this.questions.quest_5_text
								: this.questions.quest_5
						}\n` +
